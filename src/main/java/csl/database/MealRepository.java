...
public class MealRepository {
    ...
    private static final String INSERT_SQL = "insert into " + TABLE_NAME + "(name) values(:name)";
    private static final String UPDATE_SQL = "update " + TABLE_NAME + " set name = :name where Id = :id";
    ...
    public Meal getMeal(Long mealId) {
        SqlParameterSource params = new MapSqlParameterSource()
                .addValue("id", mealId);
        List<Meal> result = template.query(SELECT_SQL + " WHERE " + COL_ID + " = :id", params, new MealWrapper());
        return result.get(0);
    }

    public List<Meal> getAllMeals() {
        return template.query(SELECT_SQL, new MealWrapper());
    }
